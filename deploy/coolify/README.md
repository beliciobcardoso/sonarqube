...
| `Elasticsearch died while starting up, exit code: 78` / Bootstrap check failures (ARM64 / Oracle Cloud) | Elasticsearch bootstrap checks failed on `as` / `fsize` ulimits or system call filters | Handled in `docker-compose.yml` via `SONAR_SEARCH_JAVAADDITIONALOPTS: "-Dbootstrap.system_call_filter=false"` and `fsize`/`as` ulimits |
